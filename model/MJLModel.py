...
def build_pairs(bs, code_inputs, nl_inputs, tokenizer):
    ...
    pos_pairs = torch.empty(shape, dtype=torch.long, device=code_inputs.device)    # neg_pairs = torch.empty(shape, dtype=torch.long)
...
def augment_data(input_ids, vocab_list, tokenizer, mlm_probability=0.2):
...
class MJLModel(nn.Module):
    def __init__(self, encoder, tokenizer, args):
        ...
        self.vocab_list = [token for token in vocab_list.values() if token not in drop_token_ids]

    @torch.no_grad()
    def _data_augment(self, input_ids):
        return augment_data(input_ids, self.vocab_list, self.tokenizer, mlm_probability=0.2)
    ...
    def _compute_similar(self, queue, _vec):
        inner_products = torch.matmul(_vec, queue)  # [B*768] * [768*K] = [B*K]
        max_vals, max_indices = torch.max(inner_products, dim=1)
        similar = queue.T[max_indices]
        max_vals = max_vals.clamp(-1, 1)
        gamms = (1 - max_vals).clamp(min=0)
    ...
    def forward(self, code_inputs=None, nl_inputs=None):
        ...
        if (code_inputs is not None) and (nl_inputs is not None):
            #### cs: text-code search ####
            nl_vec = self.encoder(nl_inputs, attention_mask=nl_inputs.ne(self.pad_id))[0]
            mask = nl_inputs.ne(self.pad_id)
            denom = mask.sum(-1).clamp(min=1)[:, None]
            nl_vec = (nl_vec * mask[:, :, None]).sum(1) / denom            
            nl_vec = torch.nn.functional.normalize(nl_vec, p=2, dim=1)
            code_vec = self.encoder(code_inputs, attention_mask=code_inputs.ne(self.pad_id))[0]
            mask = code_inputs.ne(self.pad_id)
            denom = mask.sum(-1).clamp(min=1)[:, None]
            code_vec = (code_vec * mask[:, :, None]).sum(1) / denom            
            ...
            if not self.args.without_ctrd:
                ...
                outputs = self.encoder(anchor_pairs_inputs, attention_mask=anchor_pairs_inputs.ne(self.pad_id)).last_hidden_state  # [B*length*768]
        ...
        if code_inputs is not None:
            outputs = self.encoder(code_inputs, attention_mask=code_inputs.ne(self.pad_id))[0]
            outputs = (outputs * code_inputs.ne(self.pad_id)[:, :, None]).sum(1) / code_inputs.ne(self.pad_id).sum(-1)[:, None]
            return torch.nn.functional.normalize(outputs, p=2, dim=1)
        else:
            outputs = self.encoder(nl_inputs, attention_mask=nl_inputs.ne(self.pad_id))[0]
            outputs = (outputs * nl_inputs.ne(self.pad_id)[:, :, None]).sum(1) / nl_inputs.ne(self.pad_id).sum(-1)[:, None]
            return torch.nn.functional.normalize(outputs, p=2, dim=1)